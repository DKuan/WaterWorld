...
from arguments_openai import parse_args
from replay_buffer import ReplayBuffer
from model_openai import actor_agent, critic_agent
...
def make_env(scenario_name, arglist, benchmark=False):
    ...
    world = MAWaterWorld_mod(n_pursuers=2, n_evaders=n_evaders, radius=radius,
                n_poison=n_poison, obstacle_radius=0.01,
                food_reward=10, poison_reward=-1,
                encounter_reward=0.01, n_coop=n_coop,
                sensor_range=0.5, obstacle_loc=None, )
...
def get_trainers(env, obs_shape_n, action_shape_n, arglist):
    ...
    """
    actors_cur = [None for _ in range(env.agents.__len__())]
    critics_cur = [None for _ in range(env.agents.__len__())]
    actors_tar = [None for _ in range(env.agents.__len__())]
    critics_tar = [None for _ in range(env.agents.__len__())]
    optimizers_c = [None for _ in range(env.agents.__len__())]
    optimizers_a = [None for _ in range(env.agents.__len__())]
    ...
    for i in range(env.agents.__len__()):
        actors_cur[i] = actor_agent(obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
        critics_cur[i] = critic_agent(sum(obs_shape_n), sum(action_shape_n), arglist).to(arglist.device)
        actors_tar[i] = actor_agent(obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
        critics_tar[i] = critic_agent(sum(obs_shape_n), sum(action_shape_n), arglist).to(arglist.device)
...
def agents_train(arglist, game_step, update_cnt, memory, obs_size, action_size, \
    ...
    if game_step > arglist.learning_start_step and \
        ...
        for agent_idx, (actor_c, actor_t, critic_c, critic_t, opt_a, opt_c) in \
            ...
            loss_c = torch.nn.MSELoss()(q, tar_value.detach()) # bellman equation
            opt_c.zero_grad()
            loss_c.backward()
            ...
            loss_a = torch.mul(torch.tensor(-1, device=arglist.device), torch.mean(critic_c(obs_n_o, action_cur_o)))
...
def train(arglist):
    ...
    action_shape_n = [2 for _ in range(env.agents.__len__())] # no need for stop bit
    ...
    print('=2 The {} agents are inited ...'.format(env.agents.__len__()))
    print('=============================')
    ...
    reward_get_cnt = 0
    t_start = time.time()
    rew_n_old = [0.0 for _ in range(env.agents.__len__())] # set the init reward
    agent_info = [[[]]] # placeholder for benchmarking info
    episode_rewards = [0.0] # sum of rewards for all agents
    agent_rewards = [[0.0] for _ in range(env.agents.__len__())] # individual agent reward
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        if game_step > 1 and game_step % 100 == 0:   
            mean_agents_r = [round(np.mean(agent_rewards[idx][-200:-1]), 2) for idx in range(env.agents.__len__())]
            mean_ep_r = round(np.mean(episode_rewards[-200:-1]), 2)
            print(" "*38 + 'episode reward:{} agents mean reward:{} reward_get_cnt:{} var:{}'.format( \
                mean_ep_r, mean_agents_r, reward_get_cnt, round(var, 2)), end='\r')
        ...
        for episode_cnt in range(arglist.per_episode_max_len):
            ...
            action_n = np.array([a + var*np.random.randn(a.shape[0]) for a in out])
            ...
            done_n = [done for _ in range(env.agents.__len__())]
            ...
            if update_cnt > 0 and var >= 0.08: var *= arglist.var_discount
            if done or terminal:
                # del episode_step = 0
                obs_n = env.reset()
                # del agent_info.append([[]])
