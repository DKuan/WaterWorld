...
def train(arglist):
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        if game_step > 1 and game_step % 100 == 0:   
            ...
            print(" "*38 + 'episode reward:{} agents mean reward:{} reward_get_cnt:{}'.format( \
                mean_ep_r, mean_agents_r, reward_get_cnt), end='\r')
        ...
        for episode_cnt in range(arglist.per_episode_max_len):
            ...
            act_n = np.array([np.array([(a_n[0]-a_n[1])*2, (a_n[2]-a_n[3])*2]) for a_n in action_n])
            act_n = np.clip(act_n, -1, 1)

            # interact with env
            new_obs_n, rew_n, done, info_n = env.step(act_n)
